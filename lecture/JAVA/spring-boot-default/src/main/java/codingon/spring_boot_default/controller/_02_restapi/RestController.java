...
//
//
//    //http://127.0.0.1:8080/introduce/홍길동
//    @GetMapping({"/introduce/{name}"})
//    public String prac01(@PathVariable String name, Model model) {
//        System.out.println("[GET] request path variable (name) = " + name);
//
//        model.addAttribute("name", name); // 성춘향
//
//        return "_02_restapi/info";
//    }
//
//    // http://127.0.0.1:8080/introduce2/name=예은&age=123
//    @GetMapping("/introduce2")
//    public String p1(@RequestParam(value="name") String name,
//                          @RequestParam(value="age") int age, Model model) {
//
//        model.addAttribute("name", name); // s
//        model.addAttribute("age", age); // 1
...
//        // - DTO 를 사용해서 데이터 캡슐화
//        // - 14-1 폼 대비 DTO를 사용하니 코드가 깔끔해지고, 데이터 구조 쉽게 확장 가능
...
//        // - 하지만, 서버측 코드는 @RequestParam 을 사용해 데이터를 받으 (Content-Type: application/x-www-form-urlencoded)
...
//        // - JSON 데이터는 요청 본문에 있지만, @RequestBody 없는 UserDTO 는 주로 폼 데이터나 쿼리 파라미터를 바인딩하는 데 사용됨
...
//        // - 즉, RESTful API 설계에 적합하며, 클라이언트-서버 감 데이터 교환을 명확하게 함
//        // => #15-2 코드의 해결책!
//
//        System.out.println("[POST] axios and dto (name) = " + userDTO.getName());
//        System.out.println("[POST] axios and dto (age) = " + userDTO.getAge());
